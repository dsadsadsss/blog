...
const github_base = "dsadsadsss/blog";
...
// Copyright
const copyright = `&copy; 2025 ${site_name} | <a href="https://github.com/kasuganosoras/cloudflare-worker-blog" target="_blank">Github</a>`;
...
async function bloghandle(request) {
  ...
  if($_GET['p'] == undefined) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch list.json, status: ${response.status}`);
      }
      ...
      `;
       ...
       data += `<p><blockquote>获取文章列表失败，请检查控制台错误信息。</blockquote></p>`;
       data += `
              <br>
              <p class="text-right">${copyright}</p>
            </div>
